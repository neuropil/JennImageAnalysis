...
for ntsI = 1:length(NTS_sections);
    ...
    SaveImageName = char(strcat('Case_',saveName));
    ...
    CellCountData.(SaveImageName) = x.cellcount;
    
    ImageData.(SaveImageName).Cells = I;
    ImageData.(SaveImageName).RedChannel = x.Original_Image(:,:,1);
    ImageData.(SaveImageName).GreenChannel = x.Original_Image(:,:,2);
    ImageData.(SaveImageName).OriginalImage = x.Original_Image;
    ImageData.(SaveImageName).Name = NameForHeat;
    ImageData.(SaveImageName).polyborders = borders;
...
CellCountOut = char(strcat('CellCounts'));
...
SaveName = char(strcat(sampleName,'Cell_Count.mat'));
...
HeatMapOut = char(strcat('HeatMapData'));
...
structName = char(strcat('Image_Data_',sampleName,'.mat'));
...
Get_Cell_Counts(SaveName, cellCLoc)
