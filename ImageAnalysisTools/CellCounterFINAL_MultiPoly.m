...
function x = CellCounterFINAL_MultiPoly(~,~,x,varargin)
...
[SegmentsCellCount, ~] = to_get_6nTs_partition_NEW(crop_channel);
...
for segcount = 1:6
    ...
    SaveImageName = char(strcat(saveName,'_','segmentQ',num2str(segcount)));
    saveas(himage, SaveImageName,'jpg');
    ...
    CellCountData.(strcat('segmentQ',num2str(segcount))) = x.cellcount;
    
%     SaveName = char(strcat(saveName,'_','segmentQ',num2str(segcount),...
%         '_CellCount.mat'));
...
SaveName = char(strcat(saveName,'_CellCount.mat'));

save(SaveName, '-struct', 'CellCountData');
