...
function x = CellCounterFINAL_MultiPoly_test(~,~,x,varargin)
...
imageNames = listdlg('PromptString','Select Image Files',...
    'SelectionMode','multiple',...
    'ListString',tempFnames);


NTS_sections = tempFnames(imageNames);
...
for ntsI = 1:length(NTS_sections);
    ...
    ch_crop_ch = menu('Choose Channel','Red','Green','Blue');
    
    switch ch_crop_ch
        case 1
            crop_channel = Ioriginal_R;
        case 2
            crop_channel = Ioriginal_G;
        case 3
            crop_channel = resizeOimage(:,:,3);
    end
    
    [SegmentsCellCount, mask] = to_get_6nTs_partition_NEW(crop_channel);
    ...
    % CellCountData = struct;
    
    
    
    I = zeros(size(Ioriginal));
    for i = 1:size(Ioriginal_R,3);
    ...
    segs = fieldnames(SegmentsCellCount);
    ...
    for segcount = 1:6
        ...
        allcells = x.cellcores;
        
        allCellBW = bwlabel(allcells);
        
        unique(allCellBW);
        
        xcells = x.cellcores;
        xcells = uint8(xcells);
        %     x((SegmentsCellCount.(char(strcat('segmentQ',num2str(segcount))))==0))=0;
        
        % Get dimensions of segment
        [pmDx,pmDy] = size(SegmentsCellCount.(segs{segcount}));
        % Get X Y vertices of segment boundary
        singleSegBW = bwboundaries(SegmentsCellCount.(segs{segcount}));
        % Create mask from segment boundary
        singSegMask = poly2mask(singleSegBW{1,1}(:,2),singleSegBW{1,1}(:,1),pmDx,pmDy);
        
        allCellBW(singSegMask == 0) = 0;
        
        cVals = unique(allCellBW);
        uCvals = cVals(cVals > 0);
        
        cCountV = 0;
        for ci = 1:length(uCvals)
            cCountV(ci) = numel(find(allCellBW == uCvals(ci)));
        end
        
        Cv.(segs{segcount}) = cCountV;
        
        SegCellIndex = allCellBW;
        
        if ~isempty(uCvals)
            borderCells = uCvals(cCountV < 3);
            
            for i2 = 1:length(borderCells)
                SegCellIndex(SegCellIndex == borderCells(i2)) = 0;
            end
            
        end
        
        x.SegCells.(segs{segcount}) = SegCellIndex;
        
        SegVals = unique(SegCellIndex);
        SegCellCount = numel(SegVals(SegVals > 0));
        
        x.cellcount.(segs{segcount}) = SegCellCount;
        
    end
    
    L = bwlabel(x.cellcores);
    count = max(unique(L));
    x.cellcount.wholeboundary = count;
    
    for segc2 = 1:6
        %To superimpose the cells on the image do the following
        I(:,:,1) = x.SegCells.(segs{segc2}); %the red channel will now have the identified cells
        % h = figure;
        % imagesc(I);
        % colormap('gray')
        ...
        B1 = bwboundaries(SegmentsCellCount.(char(strcat('segmentQ',num2str(segc2)))));
        ...
        SaveSQImageName = char(strcat('Case_',saveName,'_','segmentQ',num2str(segcount)));
        saveas(himage, SaveSQImageName,'jpg');
        
        close all;
        
    end
    
    SaveCellCName = char(strcat('Case_',Fname));
    CellCountData.(SaveCellCName) = x.cellcount;
    
end

% Collect cellcount data
...
CellCountsAll.CellCountData = CellCountData;

SaveName = char(strcat(sampleName,'_CellCount.mat'));

save(SaveName, '-struct', 'CellCountsAll');

%% Save and Export as Excel sheet
% [FNN, PNN] = uigetfile;

% cellCLoc = strcat(PathName,CellCountOut,'\');

Get_Cell_Counts(SaveName, CellCountOut)
